...
if __name__ == "__main__":
 ...
 unvectorizer = bigram_trans(combi) 

 X_uni = unvectorizer.transform(combi)
...
# Set the parameters by cross-validation
 tuned_parameters = [{'solver' : ['lbfgs'] ,'penalty' : ['l2'], 'multi_class': ['multinomial'] ,'n_jobs': [-1], 'max_iter': [500], 'C': [1e-2, 1e-1, 1e0, 1e1] }]
 ...
 if (True):
    ...
    clf = GridSearchCV(LogisticRegression(), tuned_parameters, cv=5,scoring = {'Accuracy': make_scorer(accuracy_score)},refit='Accuracy',return_train_score=True)
